...
def add_watermark(image, text, data):
    ...
    # Text outline
    outline_color = "black"
    outline_thickness = font_size / 15  # 1/15th of the font size
...
def save_image(request_id, output_image, model_type, data, image_index=0, font_size=20):
    try:
        ...
        if model_type != "txt2video":
            watermarked_image = add_watermark(output_image, "JSCammie.com", data)
        ...
        if current_model is not None:
            ...
            if model_outputs is not None:
                ...
                if model_type != "txt2video":
                    ...
                    PIL_Images = []  # Ensure this list is defined before you fill it
...
def validate_input_data(data):
    ...
    if data['request_type'] == 'latent_couple':
        ...
        if splits is not None:
            ...
            if splits < 1 or splits > 5:
                return None, "Splits needs to be between 1 and 5"
        else:
    ...
    if data['quantity'] > 5:
        return None, "You have reached the limit of 8 images per request. Please reduce the number of images and try again."

    if data['steps'] > 20 and data['quantity'] > 5:
        return None, "You have reached the limit of 8 images per request. Please reduce the number of steps and try again."
    ...
    embedding_words = "boring_e621_v4, fcNeg, fluffynegative, badyiffymix41, gnarlysick-neg, negative_hand-neg, "
    if data.get("model", "sonic").startswith("sdxl-"):
        negative_prompt_final = data.get("negativeprompt", "")
    else:
        negative_prompt_final = embedding_words + data.get("negativeprompt", "")
