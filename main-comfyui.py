...
try:
    ...
    txt2img_models = {
        'realisticVision': {'model_path': config["realisticVision_model_path"], 'helper':None},
        ...
        'sdxl-zonkey': {'model_path': config["sdxl_zonkey_model_path"]},
...
def process_request(queue_item):
    try:
        ...
        ckpt_name = txt2img_models[data['model']]['model_path']
        ...
        print("Request JSON 260: ", request_json['260'])
                
        request_json['260']['inputs']['ckpt_name'] = ckpt_name
                
        if data['request_type'] == "img2img" or data['request_type'] == "inpainting":
            request_json['283']['inputs']['float'] = data['strength']
            # convert the PIL Image to a base64 string:
            # print type:
            request_json['277']['inputs']['image'] = f"C:\\Users\\anime\\Documents\\Coding\\JSCammie-SD-API\\process_images\\image\\image{request_id}{data['gpu_id']}.png"
        else:
            request_json['283']['inputs']['float'] = 1
            
        if data['request_type'] == "inpainting":
            request_json['279']['inputs']['image'] = f"C:\\Users\\anime\\Documents\\Coding\\JSCammie-SD-API\\process_images\\inpainting\\inpainting{request_id}{data['gpu_id']}.png"
