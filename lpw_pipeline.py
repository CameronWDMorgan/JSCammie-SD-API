    ...
    for text in prompt:
        ...
        for word, weight in texts_and_weights:
            ...
            token = pipe.tokenizer(
                word, max_length=max_length, truncation=True
            ).input_ids[1:-1]
            text_token += token
...
def get_weighted_text_embeddings(
    ...
    if uncond_prompt is not None:
        return text_embeddings, uncond_embeddings  # type: ignore
...
class StableDiffusionLongPromptWeightingPipeline(
    ...
    def __init__(
        ...
        if (
            hasattr(scheduler.config, "steps_offset")  # type: ignore
            and scheduler.config.steps_offset != 1  # type: ignore
        ):
            deprecation_message = (
        ...
        is_unet_sample_size_less_64 = (
            hasattr(unet.config, "sample_size") and unet.config.sample_size < 64  # type: ignore
        )
    ...
    def _encode_prompt(
            ...
            if isinstance(self, TextualInversionLoaderMixin):
                ...
                if do_classifier_free_guidance and negative_prompt_embeds is None:
                    negative_prompt = self.maybe_convert_prompt(
                        negative_prompt, self.tokenizer  # type: ignore
                    )
        ...
        if do_classifier_free_guidance:
            bs_embed, seq_len, _ = negative_prompt_embeds.shape  # type: ignore
    ...
    def __call__(
        ...
        image: Union[torch.FloatTensor, PIL.Image.Image, None] = None,
        mask_image: Union[torch.FloatTensor, PIL.Image.Image, None] = None,
        height: int = 512,
        width: int = 512,
        ...
        prompt_embeds: Optional[torch.FloatTensor] = None,
        negative_prompt_embeds: Optional[torch.FloatTensor] = None,
        max_embeddings_multiples: Optional[int] = 999,
        ...
        if isinstance(image, PIL.Image.Image):
            image = preprocess_image(image, batch_size)  # type: ignore
        if image is not None:
            image = image.to(device=self.device, dtype=dtype)  # type: ignore
        if isinstance(mask_image, PIL.Image.Image):
            mask_image = preprocess_mask(mask_image, batch_size, self.vae_scale_factor)  # type: ignore
        if mask_image is not None:
            mask = mask_image.to(device=self.device, dtype=dtype)  # type: ignore
            mask = torch.cat([mask] * num_images_per_prompt)  # type: ignore
        else:
            mask = None
                ...
                if mask is not None:
                    ...
                    if add_predicted_noise:
                        init_latents_proper = self.scheduler.add_noise(
                            init_latents_orig, noise_pred_uncond, torch.tensor([t])  # type: ignore
                        )
                    else:
                        init_latents_proper = self.scheduler.add_noise(
        ...
        if output_type == "latent":
            ...
            image, has_nsfw_concept = self.run_safety_checker(
                image, device, prompt_embeds.dtype  # type: ignore
            )
        ...
        return StableDiffusionPipelineOutput(  # type: ignore
